let handler = async (m, { conn, usedPrefix }) => {
  try {
    ...
    const { health, armor, rubah, kuda, kucing, anjing } = user;
    ...
    const kayu = Math.floor(Math.random() * 3) + 1; // Jumlah kayu ditingkatkan menjadi 1 hingga 3
    const batu = Math.floor(Math.random() * 2) + 1; // Jumlah batu ditingkatkan menjadi 1 hingga 2
