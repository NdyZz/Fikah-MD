...
async function handler(m, { conn, text }) {
  ...
  if (m.sender in conn.quest) {
    if (conn.quest[m.sender].currentQuestion >= conn.quest[m.sender].pertanyaan.length) {
      return m.reply("🏆 Anda telah menyelesaikan semua questgame.");
    }
    return m.reply("⏳ Anda masih memiliki questgame yang belum selesai. Silakan selesaikan terlebih dahulu.");
  } else {
    if (text === 'start') {
      ...
      if (user.healt === 0 || user.stamina === 0 || user.mana === 0) return m.reply("❗ Stamina/health/mana Anda kurang dari 100.");
      ...
      instructions += "Ketik *'questgame start'* untuk memulai game.\n";
...
handler.before = async m => {
  ...
  if (txt === 'stop') {
    m.reply("❌ Game quest telah dihentikan. Ketik *'questgame start'* untuk memulai game kembali.");
...
handler.help = ['questgame'];
handler.tags = ['rpg'];
handler.command = /^(questgame)$/i;
