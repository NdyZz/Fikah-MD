...
let handler = async (m, {
    ...
    if (!input) {
        ...
        const transformedDataResult = await Promise.all(transformedData.map(async (item, index) => ({
            ...item,
            avatar: await conn.profilePictureUrl(dataArray[index], "image").catch(_ => logo)
        })));
        ...
        if (args[0] == '1') {
            // Inventory 1
            const member = global.db.data.users[m.sender]
            let health = member.healt
...
`.trim()
            ...
            await conn.sendFile(m.chat, imgr + "INVENTORY", '', str, m, null, {
...
`.trim()
            await conn.sendFile(m.chat, imgr + "INVENTORY", '', caption, m, null, {
            ...
            let health = global.db.data.users[m.sender].healt
...
`.trim()
            ...
            await conn.sendFile(m.chat, imgr + "INVENTORY", '', aineh, m, null, {
