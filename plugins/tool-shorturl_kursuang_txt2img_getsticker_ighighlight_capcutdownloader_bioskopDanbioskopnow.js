...
`;
    ...
    try {
      ...
      if (response.ok) {
        ...
        const formattedData = `Dari: ${from}  Ke: ${to}\nHasil: ${result}`;
    ...
    if (!args[0]) throw `${usedPrefix}${command} https://blogspot.com/`;
    ...
    try {
      ...
      if (response.ok) {
        ...
        const vgd = data.vgd;
        const formattedData = `Link Asli ${args[0]}\n=====SHORT LINK=====\nBitly: ${bitly}\nIsgd: ${isgd}\nOuo: ${ouo}\nTinyURL: ${tinyurl}\nVgd: ${vgd}`;
        m.reply(formattedData);
      } else {
...
conn.reply(m.chat, 'Sedang membuat gambar...', m);
  ...
  *Usage:* ${pemakai}
  ...
  _Capcut Downloader By Fikah-MD_
  `;
